...
def _add_transcript_info(mutator, transcript, output):
    ...
    if transcript.translate:
        ...
        protein_original, res = star_subst(cds_original, protein_original, transcript, triplets, aa_dict_r, output, True)
        if res:
        ...
        if result:
             output.addMessage(__file__, 2, 'WSUBST',
                              ' The noncanonical amino acids were found and substituted in \
                                predicted protein (transcript:{0}, protein:{1}) according to translation exception annotation to original protein. \
                                !Context was not taken into account! (See table below CDS information) '.format(transcript.transcriptID, transcript.proteinID))
        ...
        output.addOutput('transExceptions', bool(transcript.transl_except))
        ...
        for transcript in gene.transcriptList:
            ...
            if not len(cds_original) % 3:
                try:
                    ...
                    triplets = define_triplet(cds_original, transcript)
                    protein_original, res = star_subst(cds_original, cds_original.translate(), transcript, triplets, aa_dict_r, output, False)
...
def star_subst(cds_original,protein, transcript, triplets, aa_dict_r, output, flag):
